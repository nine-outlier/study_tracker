...
const ColorWheelGame = ({ onComplete = () => {}, difficulty }) => {
  // ✅ Permanent “disable ads” switch (no removal, just never enable/show)
  ...
  const initRound = (r) => {
    ...
    const d = {
      ...
      // ✅ Ads hard-disabled
      sideAd: !ADS_DISABLED && Math.random() < 0.2,
      ...
      // ✅ Ads hard-disabled
      adsEnabled: !ADS_DISABLED && Math.random() < 0.03
    };
    ...
    if (d.retroMenu) {
      ...
      gameState.current.stroopMenuOptions = sliceColors.map((sc, i) => {
        const ink = sliceColors[Math.floor(Math.random() * 4)].hex;
        return {
          text: sc.name,
          ink: ink,
          realIndex: i
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // --- 1. Event Triggers ---
      // ✅ Ads hard-disabled
      if (!ADS_DISABLED && !isLocked && !adActive && d.adsEnabled) {
      ...
      if (d.handObstruct) {
        ...
        ctx.fillText(
          '👈',
          finalWx + 100 + gameState.current.handPosition.x,
          finalWy + gameState.current.handPosition.y
        );
      }
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-8 pointer-events-none">
        ...
        <div className="text-center">
          ...
          <div
            className={`text-sm font-bold tracking-[0.3em] mt-2 opacity-50 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}
          >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
          <div ref={textRef} className="flex flex-col items-center justify-center transition-transform duration-75 will-change-transform">
            ...
            {message && (
              <h1
                className={`text-7xl font-black drop-shadow-[0_0_15px_rgba(255,255,255,0.8)] animate-bounce ${
      ...
      {!ADS_DISABLED && adActive && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm pointer-events-auto animate-in fade-in zoom-in duration-200">
          <div className="bg-white p-8 max-w-md text-center border-4 border-red-500 shadow-[0_0_50px_rgba(239,68,68,0.5)] rotate-2">
            ...
            <div className="text-xs font-bold text-slate-400 tracking-widest animate-pulse">PRESS [SPACE] TO DISMISS</div>
          </div>
        </div>
      )}
    </div>
