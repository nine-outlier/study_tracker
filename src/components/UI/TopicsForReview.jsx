...
const TopicsForReview = ({ topics, isWeighted, appSettings, headerRight = null }) => {
  ...
  return (
    <div className="app-bg-surface rounded-xl shadow-sm border app-border-muted p-4 sm:p-6">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold app-text-main">Domains To Review</h2>
          <p className="text-sm app-text-muted">
            All domains with a {isWeighted ? 'weighted' : 'raw'} average below passing ({config.PASSING_SCORE}%),
            ranked by priority.
          </p>
        </div>
      ...
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
        {safeTopics.length > 0 ? safeTopics.map((area, idx) => {
