...
import { getHighContrastColor, getTrendColorClass } from '../../utils/themeHelpers.js';
import ChartGradientDefs from '../UI/ChartGradientDefs.jsx';

const PerformanceTrends = ({
  trendData, rawTrendStats, weightedTrendStats,
  useWeightedAverages, setUseWeightedAverages,
  trendFilter, setTrendFilter,
  weights, setWeights,
  appSettings, rankingEngine
}) => {
  const [showCustomize, setShowCustomize] = useState(false);

  const contrastColor = getHighContrastColor(appSettings?.theme ?? appSettings);

  const CustomDot = (props) => {
    const { cx, cy, stroke } = props;
    return <circle cx={cx} cy={cy} r={5} fill={contrastColor} stroke={stroke || contrastColor} strokeWidth={2} />;
  };
  ...
  const meanRank = trendStats.meanRank || (rankingEngine ? rankingEngine.determineRank(trendStats.mean) : 'Critical');
  const medianRank = trendStats.medianRank || (rankingEngine ? rankingEngine.determineRank(trendStats.median) : 'Critical');

  const meanColor = rankingEngine ? rankingEngine.getRankClass(meanRank) : 'app-text-chart-1';
  const medianColor = rankingEngine ? rankingEngine.getRankClass(medianRank) : 'app-text-chart-1';
