...
/**
 * GeneralSettingsForm: Sub-component for SettingsModal.
 */
  ...
  const SettingToggle = ({ id, label, description, checked, onChange }) => (
    <div className="flex justify-between items-start">
      <div>
        <label
          htmlFor={id}
          className="text-md font-medium text-slate-800 dark:text-slate-100"
        >
          {label}
        </label>
  ...
  const SettingSelect = ({ id, label, value, onChange, children }) => (
    <div className="flex justify-between items-center">
      ...
      <select
        ...
        value={value}
        onChange={onChange}
        className="w-1/2 px-3 py-2 border border-slate-300 rounded-md text-sm dark:bg-gray-800 dark:border-gray-700 dark:text-slate-100"
      >
        {children}
      </select>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6 p-4 bg-slate-50 rounded-lg border border-slate-200 dark:bg-gray-800 dark:border-gray-700">
        <SettingToggle
          id="darkMode"
          label="Dark Mode"
          description="Enable high-contrast dark theme."
          checked={appSettings.darkMode}
          onChange={() => handleSettingChange('darkMode', !appSettings.darkMode)}
        />
        ...
        <SettingToggle
          ...
          description="Use Lexend, a font with wider spacing for readability."
          checked={appSettings.useAccessibleFont}
          onChange={() =>
            handleSettingChange('useAccessibleFont', !appSettings.useAccessibleFont)
          }
        />
...
/**
 * DataManagementForm: Sub-component for SettingsModal.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-red-50 border border-red-200 rounded-lg dark:bg-red-900/30 dark:border-red-500/50">
        <h4 className="text-md font-medium text-red-800 dark:text-red-300">
          Permanent Deletion
        </h4>
        <p className="mt-2 text-sm text-red-700 dark:text-red-400">
          This action will permanently delete all tests, sessions, and domains
          that you have previously &quot;soft deleted&quot;. This cannot be
          undone.
        </p>
        <button
          type="button"
...
/**
 * SettingsModal: Modal for all application settings.
 */
const SettingsModal = ({
  isVisible,
  onClose,
    ...
    >
      ...
      >
        ...
        <div className="max-h-[70vh] overflow-y-auto pr-2">
          ...
          {formType === 'data' && (
            <DataManagementForm onPromptPurge={onPromptPurge} />
          )}
        </div>
      </div>
    </div>
